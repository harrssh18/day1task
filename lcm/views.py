	...
	try:
		if request.method == "POST":
			...
			if fm.is_valid():
				...
				return redirect(f'/lcm-of-{num1}-and-{num2}/')
		else:
			fm= LCMForm()
...
def result(request,num1,num2):
	if LCM.objects.filter(inputs=str(num1)+'x'+str(num2)).exists():
		...
		lcm = pi.finalresult
		prime_fact1 = pi.firstfact
		prime_fact2 = pi.secondfact
		l = pi.li
	else:
	...
	context = {'lcm':lcm,'l':l,'prime_fact1':prime_fact1,'prime_fact2':prime_fact2,'num1':num1,'num2':num2}
	return render(request,'lcm/result.html',context)
